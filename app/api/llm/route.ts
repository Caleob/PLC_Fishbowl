...
// Allowed model IDs — reject anything else to prevent abuse
const ALLOWED_MODELS = new Set([
  'gemini-3.1-flash-lite',
]);

// Max request body size (1MB — generous for long transcripts)
const MAX_BODY_SIZE = 1_000_000;
const MAX_MESSAGES = 64;
const MAX_MESSAGE_CHARS = 300_000;
const MAX_TOTAL_MESSAGE_CHARS = 600_000;
...
export async function POST(req: NextRequest) {
  ...
  const allowServerProxy = process.env.ALLOW_SERVER_GEMINI_PROXY === 'true' || process.env.ALLOW_SERVER_ANTHROPIC_PROXY === 'true';
  ...
  if (isHostedMode && modelId !== HOSTED_MODEL_ID) {
    return jsonError('Hosted Fishbowl sessions only support Gemini 3.1 Flash-Lite.', 400);
  }
  ...
  const apiKey = isHostedMode
    ? (serverApiKey || '')
    : (body.apiKey || (allowServerProxy ? serverApiKey || '' : ''));
  if (!apiKey) {
    return jsonError(
      isHostedMode
        ? `No API key configured. Set GEMINI_API_KEY on the server.`
        : `No API key configured. Enter your Gemini API key, use local, or opt in to server proxy.`,
      400,
    );
  }
  ...
  try {
    if (provider === 'gemini') {
      return await handleGemini(messages, apiKey, effectiveModelId, stream);
    } else {
...
// Claude API integration removed.

async function handleGemini(messages: { role: string; content: string }[], apiKey: string, modelId: string, stream?: boolean) {
  ...
  const streamParams = stream ? '&alt=sse' : '';
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${modelId}:${endpoint}?key=${apiKey}${streamParams}`;
  ...
  if (promptTokens > 0 || candidateTokens > 0) {
    recordSpend(modelId, promptTokens, candidateTokens).catch(() => {});
  }
