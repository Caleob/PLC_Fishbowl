...
export class ConversationOrchestrator {
  ...
  prefetchInitialTakes(): void {
    ...
    for (let idx = 0; idx < this.panelists.length; idx++) {
      ...
      const prompt = `${ideaContext}\n\nYou are panelist ${idx + 1} of ${totalPanelists}. React naturally, like you just read the lesson transcript and context for the first time. Focus specifically on what your expertise as a ${panelist.role} reveals about the teaching strategy, student engagement, and lesson delivery that a generalist would miss. Don't give a broad overview. Go deep on the one or two pedagogical elements that matter most from YOUR perspective. Be specific and draw on your expertise. Keep your response to 100-200 words. Write in plain text only. No markdown, no em-dashes, no formatting.`;
  ...
  prefetchCrossTalk(): void {
    ...
    (async () => {
      ...
      for (let idx = 0; idx < this.panelists.length; idx++) {
        ...
        const prompt = `${ideaContext}\n\n${transcriptContext}\n\n${roundContext}Jump in like you would in a real pedagogical coaching meeting. Push back, challenge, or build on what was said about the lesson. Reference specific panelists by name. Do NOT restate or summarize points that were already made. If you agree with something, say so in one line and move on to a NEW pedagogical point or suggestion. Bring something to the table that hasn't been said yet. Keep your response to 100-200 words. Write in plain text only. No markdown, no em-dashes, no formatting.`;
  ...
  prefetchModerationResponses(question: string): void {
    ...
    const ideaContext = this.buildIdeaContext();
    const transcriptContext = this.buildTranscriptContext();
    const prompt = `${ideaContext}\n\n${transcriptContext}\n\nModerator: ${question}\n\nThe moderator has stepped in with a question or directive. Respond directly to what they asked. You can reference what other panelists have said but do NOT repeat their points. If a previous panelist already answered part of the question, acknowledge it briefly and add something new from your pedagogical perspective. Be specific and actionable. Keep your response to 100-200 words. Write in plain text only. No markdown, no em-dashes, no formatting.`;
  ...
  private buildIdeaContext(): string {
    let context = '';
    if (this.ideaText) {
      context += `Lesson Topic/Objectives/Goals:\n\n${this.ideaText}\n\n`;
    }
    for (const file of this.ideaFiles) {
      context += `Transcript/Document: ${file.name}\n\n${file.content}\n\n`;
    }
    return context;
  }
  ...
  private async _runSinglePanelist(panelist: Panelist, round: RoundType): Promise<void> {
    ...
    if (round === 'initial-takes') {
      ...
      prompt = `${ideaContext}\n\nYou are panelist ${panelistIdx + 1} of ${this.panelists.length}. React naturally, like you just read the lesson transcript and context for the first time. Focus specifically on what your expertise as a ${panelist.role} reveals about the teaching strategy, student engagement, and lesson delivery that a generalist would miss. Don't give a broad overview. Go deep on the one or two pedagogical elements that matter most from YOUR perspective. Be specific and draw on your expertise. Keep your response to 100-200 words. Write in plain text only. No markdown, no em-dashes, no formatting.`;
    } else if (round === 'cross-talk') {
      prompt = `${ideaContext}\n\n${transcriptContext}\n\nJump in like you would in a real pedagogical coaching meeting. Push back, challenge, or build on what was said about the lesson. Reference specific panelists by name. Do NOT restate or summarize points that were already made. If you agree with something, say so in one line and move on to a NEW pedagogical point or suggestion. Bring something to the table that hasn't been said yet. Keep your response to 100-200 words. Write in plain text only. No markdown, no em-dashes, no formatting.`;
    } else if (round === 'wrap-up') {
      prompt = `${transcriptContext}\n\nThe discussion is wrapping up. The most urgent need is advice on improving this lesson (and the teacher's lessons in general). State your single most important, actionable recommendation: suggest one thing to add, one thing to leave out, or a powerful strategy to try. Keep it concise, supportive, and real. No markdown, no em-dashes, just plain talk.`;
    } else {
  ...
  async runModerationPanelist(panelist: Panelist, question: string): Promise<void> {
    ...
    const transcriptContext = this.buildTranscriptContext();
    const prompt = `${ideaContext}\n\n${transcriptContext}\n\nModerator: ${question}\n\nThe moderator has stepped in with a question or directive. Respond directly to what they asked. You can react to what other panelists have said, but do NOT repeat their points. If someone already made the point you were going to make, acknowledge it briefly ("Like Mr. Spock said...") and move to a NEW angle. Bring your unique pedagogical expertise. Be specific and actionable. Keep your response to 100-200 words. Write in plain text only. No markdown, no em-dashes, no formatting.`;
