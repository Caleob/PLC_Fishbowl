...
const SUGGEST_PROMPT = `Read the document(s) below and extract a single short topic line — the kind of one-sentence description a person would put in a "what lesson context should the panel evaluate?" box.
...
- Examples of good output:
    "Introductory lesson on logarithmic scaling for high school physics"
    "Middle school STEAM lesson exploring plant biology structures"
    "Review session on kinetic energy and Newton's laws"
...
export default function IdeaInput({ ideaText, ideaFiles, onTextChange, onFilesChange }: Props) {
  ...
  const handleSuggestTopic = useCallback(async () => {
    ...
    try {
      const body: Record<string, unknown> = {
        ...
        provider: provider,
        // Hosted mode only allows Sonnet; BYOK can use Haiku/Flash for speed/cost.
        modelId: isHosted ? HOSTED_MODEL_ID : (provider === 'gemini' ? 'gemini-1.5-flash' : 'claude-haiku-4-5-20251001'),
        stream: false,
      };
      if (isHosted) {
  ...
  const handleFilesWithFeedback = useCallback(async (files: FileList | File[]) => {
    ...
    const supported = new Set(['md', 'txt', 'pdf', 'vtt']);
    const unsupported = fileArray.filter((f) => { const ext = f.name.split('.').pop()?.toLowerCase(); return !ext || !supported.has(ext); });

    const messages: string[] = [];
    if (unsupported.length > 0) {
      messages.push(`Unsupported: ${unsupported.map((f) => f.name).join(', ')}. Supported: .pdf, .md, .txt, .vtt`);
    }
    ...
    >
      ...
      <div className="p-4" style={{ background: 'var(--bg-card)' }}>
        ...
        <textarea
          ...
          placeholder="What lesson concepts, objectives, or teaching goals are we evaluating? Or paste a snippet of your lesson transcript..."
          className="w-full h-36 resize-none text-sm leading-relaxed mission-briefing-textarea"
          aria-label="Your lesson topic or context"
        ...
        <div className="px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
              Submit .pdf, .md, .txt, or .vtt — or{' '}
              <label className="cursor-pointer" style={{ color: 'var(--accent-gold)' }}>
                browse
                <input type="file" accept=".pdf,.md,.txt,.vtt" multiple onChange={handleFileSelect} className="hidden" />
              </label>
            </p>
