...
const SUPPORTED_EXTENSIONS = new Set(['md', 'txt', 'pdf', 'vtt']);
...
function cleanVttContent(rawContent: string): string {
  ...
  for (const line of lines) {
    ...
    if (trimmed.toUpperCase() === 'WEBVTT') continue;

    // Ignore cue numbers (numbers only)
    if (/^\d+$/.test(trimmed)) continue;

    // Ignore timestamp lines (contain -->)
    if (trimmed.includes('-->')) continue;

    // Strip speaker names (e.g. "Caleob King: Hello" -> "Hello")
    let text = trimmed;
    const nameMatch = text.match(/^([A-Za-z\s'\-]+):\s*(.*)$/);
    if (nameMatch) {
      text = nameMatch[2]; // get the content after the speaker name
    }

    if (text.trim()) {
      cleanLines.push(text.trim());
    }
  }
...
export async function parseFiles(files: FileList | File[]): Promise<ParseResult> {
  ...
  for (const file of fileArray) {
    ...
    try {
      let content = ext === 'pdf' ? await extractPdfText(file) : await file.text();
      if (content.trim()) {
        const isVtt = ext === 'vtt' || content.trim().startsWith('WEBVTT') || content.includes('-->');
        if (isVtt) {
          content = cleanVttContent(content);
        }
